...
static inline GLfloat shoelace(const vec2 a, const vec2 b) { return (b.x - a.x) * (a.y + b.y); }
...
static int __add_cubic_size(const FT_Vector *control1, const FT_Vector *control2, const FT_Vector *to, void *s) {
...
int msdfgl_glyph_buffer_size(FT_Face face, int code, size_t *meta_size, size_t *point_size) {
    ...
    if (FT_Outline_Decompose(&face->glyph->outline, &fns, &ctx)) return -1;
    ...
    for (int i = 0; i < ncontours; ++i) {
        ...
        if (nsegments) {
            ...
            for (int j = 0; j < nsegments; ++j, ++index) {
                ...
                if (is_corner(normalize(prev_direction), normalize(cur_direction), cross_threshold))
