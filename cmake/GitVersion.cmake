...
set(VERSION_DEV_FLAG dev)     # Development identifier
set(DEV_BRANCH develop)       # Development branch name
...
function(get_version_info prefix directory)
  ...
  if(GIT_FOUND)
    ...
    execute_process(COMMAND ${GIT_EXECUTABLE} describe --tags --match "${VERSION_TAG_PREFIX}[[:digit:]]*.[[:digit:]]*.[[:digit:]]*" --exclude "v*-dev*"
    ...
    if(result EQUAL 0)
      ...
      #message(STATUS "Fetched tag ${GIT_TAG_VERSION}")
    ...
    if(result EQUAL 0)
      ...
      string(COMPARE EQUAL "main" ${GIT_BRANCH} ON_MASTER)
      # Check for develop branch
      string(COMPARE EQUAL ${DEV_BRANCH} ${GIT_BRANCH} ON_DEV)
      
      if(DEFINED ENV{DEVELOP_BRANCH})
        set(ON_DEV True)
      endif()
      ...
      if(ON_RELEASE_BRANCH)
        ...
        execute_process(COMMAND ${GIT_EXECUTABLE} describe --tags --match "${VERSION_RC_START_TAG_PREFIX}[[:digit:]]*.[[:digit:]]*.[[:digit:]]*" --exclude "v*-dev*"
          RESULT_VARIABLE result
    ...
    if(NOT ON_MASTER AND NOT ON_RELEASE_BRANCH AND NOT ON_HOTFIX_BRANCH AND NOT ON_DEV)
      # Auto increment version number, set alpha flag
      #MATH(EXPR ${prefix}_VERSION_MINOR "${${prefix}_VERSION_MINOR}+1")
      #set(${prefix}_VERSION_PATCH 0)
      set(${prefix}_VERSION_FLAG ${VERSION_ALPHA_FLAG})
    elseif(ON_DEV)
      set(${prefix}_VERSION_FLAG ${VERSION_DEV_FLAG})
    endif()
  ...
  #message(FATAL_ERROR ${prefix}_VERSION_PATCH)
  ...
  write_file(baguette_version "${VERSION_STRING}")
endfunction()
