...
#define IMGL3W_IMPL
#include "imgui_backend/imgui_impl_opengl3_loader.h"
#endif
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version) {
    ...
    if (const char* vendor = (const char*)glGetString(GL_VENDOR))
        if (strncmp(vendor, "Intel", 5) == 0) bd->UseBufferSubData = true;
            ...
            // minor, (const char*)glGetString(GL_VENDOR), (const char*)glGetString(GL_RENDERER)); // [DEBUG]
#else
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data) {
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        // - On Intel windows drivers we got reports that regular glBufferData() led to accumulating leaks when using
        // multi-viewports, so we started using orphaning + glBufferSubData(). (See
        // https://github.com/ocornut/imgui/issues/4468)
        // - On NVIDIA drivers we got reports that using orphaning + glBufferSubData() led to glitches when using
        // multi-viewports.
        ...
        if (bd->UseBufferSubData) {
            ...
            glBufferData(GL_ELEMENT_ARRAY_BUFFER, idx_buffer_size, (const GLvoid*)cmd_list->IdxBuffer.Data,
                         GL_STREAM_DRAW);
        }
