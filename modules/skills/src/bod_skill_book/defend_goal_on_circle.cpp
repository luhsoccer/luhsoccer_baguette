...
void DefendGoalOnCircleBuild::buildImpl(const config_provider::ConfigStore& cs) {
    ...
    auto calc_y_on_goal = [enemy_ball_carrier](const std::shared_ptr<const transform::WorldModel>& wm,
        ...
        if ((b_pos->translation() - e_pos->translation()).norm() > 0.5) return std::nullopt;
    ...
    DoubleComponentParam weight(
        ...
        [&cs, calc_y_on_goal](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            ...
            if (std::abs(y_on_goal_line.value()) < cs.skills_config.defend_goal_on_circle_threshold) return 1.0;
            return 0.0;
        });
    DoubleComponentParam weight_negated(
        CALLBACK, [weight](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
    ...
    defend_goal.addFeature(
        TargetFeature(LineShape(y_on_goal_line_point, "ball"), cs.skills_config.translational_tolerance, weight, true));
    ...
    defend_goal.addFeature(TargetFeature(LineShape(ball_pos, goal_center), cs.skills_config.translational_tolerance,
                                         weight_negated, true));
