...
void KickBallThroughTargetBuild::buildImpl(const config_provider::ConfigStore& cs) {
    ...
    ConditionStep ball_to_fast(
        {CALLBACK, [](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> bool {
             ...
             if (ball_vel.has_value() && ball_vel->velocity.norm() > 0.6) return true;
             return false;
         }});
    ...
    pass_ball_up.addFeature(
        TargetFeature(LineShape(past_ball_pos, "ball", -10, cs.skills_config.move_to_ball_pre_radius)));
    ...
    receive_ball.setCancelCondition(
        {CALLBACK, [](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> bool {
             auto ball_vel = wm->getVelocity("ball");
             if (ball_vel.has_value() && ball_vel->velocity.norm() < 0.3) return true;
             return false;
         }});
    ...
    drive_in_ball.addFeature(BallTargetFeature(CircleShape("ball", 0.03, true), 0.1, false));
