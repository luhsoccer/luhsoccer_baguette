...
void BackwardsDribblingBuild::buildImpl(const config_provider::ConfigStore& cs) {
    ...
    BoolComponentParam drove_too_far(
        CALLBACK, [target_point](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            ...
            auto ballinfo = wm->getBallInfo();
            if (ballinfo.has_value() && !(ballinfo->state == transform::BallState::IN_ROBOT)) {
                return true;
            }
    ...
    preposition_to_ball.setCancelCondition(
        {CALLBACK, [](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> bool {
             ...
             if (ball_info.has_value()) {
                 return ball_info->isInRobot(td.robot);
             }
             return false;
         }});
    ...
    // get_ball.setMaxVelX(cs.local_planner_components_config.robot_vel_max_x *
    // cs.skills_config.max_vel_drive_dribbling); get_ball.activateConstraint(DriveStepConstraintNames::BALL_PLACEMENT,
    // false); get_ball.setAvoidDefenseArea(false); get_ball.setAvoidOtherRobots(false); addStep(std::move(get_ball));
    ...
    drive_in_ball.activateConstraint(DriveStepConstraintNames::BALL_PLACEMENT, false);
    drive_in_ball.setAvoidDefenseArea(false);
    ...
    addStep(WaitStep(WAIT_DURATION, cs.skills_config.dribbling_wait_after_dribbler_high));
    ...
    go_to_target.setAvoidOtherRobots(false);
    ...
    ConditionStep c({CALLBACK, [](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> bool {
                         auto target_comp = ComponentPosition({TD_Pos::POINT, 0});
                         auto t_pos = target_comp.positionObject(wm, td).getCurrentPosition(wm);
                         auto ball_comp = ComponentPosition("ball");
                         auto b_pos = ball_comp.positionObject(wm, td).getCurrentPosition(wm);

                         if (b_pos.has_value() && t_pos.has_value()) {
                             if ((b_pos->translation() - t_pos->translation()).norm() <= 0.15 && td.required_bools[0]) {
                                 return true;
                             }
                         }
                         return false;
    ...
    c.addIfStep(WaitStep(WAIT_DURATION, cs.skills_config.wait_after_dribbling));
    ...
    get_away.setAvoidOtherRobots(false);
