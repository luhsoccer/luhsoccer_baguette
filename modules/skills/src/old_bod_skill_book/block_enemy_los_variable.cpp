...
BlockEnemyLosVariableBuild::BlockEnemyLosVariableBuild()
    : SkillBuilder("BlockEnemyLosVariable",  //
                   {"EnemyBallCarrier"},     //
                   {},                       //
                   {"radius"},               //
                   {},                       //
...
void BlockEnemyLosVariableBuild::buildImpl(const config_provider::ConfigStore& cs) {
    ...
    auto fieldline_puffer = [&cs](const std::shared_ptr<const transform::WorldModel>& wm,
        ...
        double puffer = cs.skills_config.missing_field_edge + cs.local_planner_components_config.robot_radius +
                        DoubleComponentParam(TD, 0).val(wm, td);
        return puffer;
    };
    ...
    DoubleComponentParam line_cutoff_start(
        CALLBACK, [&cs](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            return -DoubleComponentParam(TD, 0).val(wm, td) * 1.1;
        });
        ...
        CALLBACK, [&cs](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            ...
            if (ball.has_value() && enemy.has_value())
                return (ball->translation() - enemy->translation()).norm() +
                       (DoubleComponentParam(TD, 0).val(wm, td) * 0.9);
            return DoubleComponentParam(TD, 0).val(wm, td) * 0.9;
        });

    ComponentPosition lineofsight_object(
        CALLBACK,
        [fieldline_puffer](const std::shared_ptr<const transform::WorldModel>& wm,
            ...
            if (ball.has_value() && enemy.has_value() && corner_ally_left.has_value() &&
                ...
                if (abs(ball->translation().x()) >
                    ...
                    double distance_s =
                        2 * sqrt(2 * DoubleComponentParam(TD, 0).val(wm, td) * distance_h - pow(distance_h, 2));
    ...
    DoubleComponentParam circle_weight = cs.skills_config.circle_weight;
    drive_to_line_of_sight.addFeature(
        TargetFeature(CircleShape("ball", {TD, 0}, false), cs.skills_config.translational_tolerance, circle_weight));
