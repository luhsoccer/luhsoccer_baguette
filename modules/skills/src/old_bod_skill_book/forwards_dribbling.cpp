...
ForwardsDribblingBuild::ForwardsDribblingBuild()
    : SkillBuilder("ForwardsDribbling",  //
                   {},                   //
                   {"TargetPoint"},      //
                   {},                   //
                   {},                   //
...
void ForwardsDribblingBuild::buildImpl(const config_provider::ConfigStore& cs) {
    // PARAMS
    ...
    BoolComponentParam drove_too_far(
        CALLBACK,
        [&cs, target_point](const std::shared_ptr<const transform::WorldModel>& wm, const TaskData& td) -> double {
            auto robot_pos =
            ...
            const double distance_puffer = cs.local_planner_components_config.robot_radius;
    ...
    get_ball.setRotationControl(HeadingRotationControl(target_point, false));
    ...
    go_to_target.setRotationControl(HeadingRotationControl(target_point));
